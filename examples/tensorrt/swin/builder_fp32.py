...
def swin_transformer(network, config, args, input_img, weights_dict):
    ...
    sw_weights = SwinTransformerWeightTransposeQKVWeight(len(depths), config.MODEL.SWIN.WINDOW_SIZE, depths, num_heads, args.th_path, weights_dict)   
...
def build_engine(config, args, weights_dict):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(explicit_batch_flag) as network, builder.create_builder_config() as builder_config:
        ...
        # Create the network
        input_img = network.add_input(name="input_img", dtype=trt.float32, shape=(-1, config.MODEL.SWIN.IN_CHANS, config.DATA.IMG_SIZE, config.DATA.IMG_SIZE))
