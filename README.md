...
## 运行步骤
由于docker容器中的cuda版本和TRT版本都不适用于工程，所以建议不使用dacker，自行安装TRT8.4.3.1和cuda11.6版本，在主机上构建环境.
...
该部分修改禁用了Patch Merging，禁用的原因为：在网络结构图中的一个Stage是由Patch merging + Swin-Block组成的，但是
FasterTransformer中的实现是Swin-Block + Patch merging，而Swin-Block的输出需要传递给上采样部分，Patch merging的优化效果相比Swin-Block会显得非常小，所以没有对Patch merging进行优化。
